...
class FbBackend:
    ...
    def __init__(self, canvas, fb, fb_var=b""):
        self.canvas, self.fb, self.fb_var = canvas, fb, fb_var
    ...
    def show(self, what):
        ...
        if self.fb is not None:
            ...
            refresh(self.fb.fileno(), self.fb_var)
...
def main():
    ...
    try:
        ...
        # a failed run gives the console back whatever --text-mode says: the script's next step or its
        # failure message must be seen
        backend.finish(args.text_mode or failed)
        if tty is not None:
            if (args.text_mode or failed) and args.backend == "fb":
                try:
                    fcntl.ioctl(tty, KDSETMODE, KD_TEXT)
                except OSError:
                    pass
